...
void TemperatureCard::draw(Display& display) {
    ...
    Text(d_x + 2 * PADDING, d_y + 2 * PADDING, d_width - 2 * PADDING,
         d_height - 2 * PADDING, temperatureTxt, TextStyle(5, Color::WHITE, Color::BLUE))
        .render(display);

    Text(d_x + 2 * PADDING, d_y + d_height - 2 * PADDING, d_width - 2 * PADDING,
         d_height - 2 * PADDING, "Temperature", TextStyle(2, Color::WHITE, Color::BLUE))
        .render(display);
}

void TemperatureCard::loop(int t) { invalidate(); }
